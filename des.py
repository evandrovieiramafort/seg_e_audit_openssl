...
def derivar_chave_des(senha: str) -> bytes:
    """Deriva uma chave de 8 bytes com MD5 para o DES."""
    return hashlib.md5(senha.encode('utf-8')).digest()[:8]

def cifrar_texto_des(texto_plano: str, senha: str) -> bytes:
    """Criptografa usando DES e retorna os bytes cifrados."""
    chave = derivar_chave_des(senha)
...
def decifrar_dados_des(dados_cifrados: bytes, senha: str) -> str:
    """Decifra dados com DES e retorna o texto original."""
    chave = derivar_chave_des(senha)
    cifrador = DES.new(chave, DES.MODE_ECB)
...
def menu_des(limpar_tela_func):
    """Menu para operações de criptografia DES."""
    while True:
        limpar_tela_func()
        ...
        if escolha == '1':
            try:
                ...
                arquivo_saida = "cifrado_des.txt"
                dados_cifrados = cifrar_texto_des(texto, senha)
            ...
            try:
                limpar_tela_func()
                print("--- Decifragem de Texto (DES) ---")
                arquivo_entrada = "cifrado_des.txt"
                ...
                texto_decifrado = decifrar_dados_des(dados_cifrados, senha)
